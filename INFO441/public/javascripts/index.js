...
async function getItinerary() {
    document.getElementById("display").innerHTML = "Creating itinerary...";
    let location = document.getElementById("location").value;
    let day = document.getElementById("day").value;
    let time = document.getElementById("time").value;
    let budget = document.getElementById("budget").value;
    let idJson = await fetchJSON('/itineraries/new_id');
    const itinerary_id = idJson.itinerary_id;
    let responseJson = await fetchJSON(`/itineraries?id=${itinerary_id}&location=${encodeURIComponent(location)}
    &day=${encodeURIComponent(day)}&budget=${encodeURIComponent(budget)}&time=${encodeURIComponent(time)}`);

    // Set global itinerary for current quiz
    itinerary = responseJson;

    document.getElementById('display').innerHTML = getItineraryHtml(itinerary);
}

async function createItinerary() {
    await fetchJSON(`/itineraries`, {
        method: "POST",
        body: itinerary
    });
    if (itinerary.Event_0) {
        document.getElementById('save_button').innerHTML = `<div class="alert alert-success" role="alert">
        Success!
      </div>`
    }
}
